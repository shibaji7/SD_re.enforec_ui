...
def get_one_day_data(date, rad="fhe"):
    startDate = date
    endDate = date + datetime.timedelta(hours=1)
    inpDir = "/sd-data/parquet/2012/%s/"%rad
    fList = []
    while startDate <= endDate:
        fList.append( inpDir + startDate.strftime("%Y%m%d") + "%s.parquet"%rad )
        startDate += datetime.timedelta(days=1)
        pass
    spO = SDParquetUtils(fList,readColList=["time", "bmnum", "v", "p_l", "w_l","slist"])
    df = spO.get_data(flattenDF=True, flattenCols=["v", "w_l", "p_l", "slist"])
    return df
